    ...
    public class VendaModel
    {
        ...
        
        {
            ...
            for (int i = 0; i < lista_produto.Count; i++)
            {
                ...
                //Baixa o produto so estoque
                sql = "update produto" +
                    $" set quantidade_estoque = (quantidade_estoque - " + int.Parse(lista_produto[i].QtdeProduto.ToString()) + ") "+
                    $" where id = { lista_produto[i].CodigoProduto.ToString()}";
                objDAL.ExecutarComandoSql(sql);
            }
        }
    }
